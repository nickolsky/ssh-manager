#!/usr/bin/env bash
# @name AmneziaWG (Docker) — Ubuntu / Debian / CentOS
# @name_en AmneziaWG (Docker) — Ubuntu / Debian / CentOS
# @group VPN
...
main(){
  ...
  pkg_install ca-certificates curl openssl iproute2 gzip
  pkg_install qrencode || warn "qrencode is not available: no QR codes in this output (SSH Manager still shows them)"
  ensure_docker
  ...
  for n in "${NAMES[@]}"; do
    ...
    echo ""
    ! cmd qrencode || qrencode -t utf8 < "$DIR/clients/${n}.conf" || true
